...
def convert_examples_to_features(js, tokenizer, args):
    ...
    return InputFeatures(source_tokens, source_ids, position_idx, dfg_to_code, dfg_to_dfg, None, type2label[js['label']])
...
def test(args, model, tokenizer, is_write_preds=False):
    ...
    if is_write_preds:
        ...
        with open(os.path.join(args.output_dir, "test_pred.pkl"), 'wb') as f:
...
def main():
    ...
    if args.do_test and args.local_rank in [-1, 0]:
        ...
        result =  test(args, model, tokenizer, is_write_preds=True)
        logger.info("***** Eval results *****")
        for key in sorted(result.keys()):
            logger.info("  %s = %s", key, str(round(result[key], 4)))
