            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if results["eval_acc"] >= best_acc:
                        ...
                        logger.info("Not improve for %s times", not_improve)
...
def evaluate(args, model, tokenizer, eval_when_training=False, is_write_pred=False):
...
def main():
    ...
    if args.do_eval:
        ...
        logger.info(f"{params * 4 / 1e6} MB model size")
        model.to(args.device)
        evaluate(args, model, tokenizer, is_write_pred=True)
