...
def evaluate(args, model, eval_examples, eval_data):
    ...
    print("Evaluate-{}...".format(args.test_filename))
    ...
    for batch in tqdm(eval_dataloader, total=len(eval_dataloader), desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            y_preds.append(logit.argmax(axis=1).cpu())
        nb_eval_steps += 1
    ...
    eval_acc = torch.mean(y_trues.eq(y_preds).float())
...
def main():
    ...
    parser.add_argument("--dataset_name", default="POJ104", type=str, required=True,
    ...
    eval_examples, eval_data = load_and_cache_classification_data(args, args.test_filename, pool, tokenizer,
                                                                  '{}_test'.format(args.dataset_name),
                                                                  False)
