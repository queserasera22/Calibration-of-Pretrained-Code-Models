...
def convert_examples_to_features(js, tokenizer, args):
    ...
    dfg, index_table, code_tokens = extract_dataflow(code, args.lang)
    ...
    return InputFeatures(source_tokens, source_ids, position_idx, dfg_to_code, dfg_to_dfg, js['label'])
...
def test(args, model, tokenizer, is_write_preds=False):
    # Loop to handle MNLI double evaluation (matched, mis-matched)
...
def main():
    ...
    parser.add_argument("--label_smoothing", type=float, default=0.0, help="The label smoothing epsilon to apply (zero means no label smoothing).")
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        output_dir = os.path.join(args.output_dir, args.dataset, '{}'.format(checkpoint_prefix))
    ...
    if args.do_test and args.local_rank in [-1, 0]:
        ...
        test(args, model, tokenizer, is_write_preds=True)
