...
def evaluate(data_path, batch_size, is_write_preds=False):
    ...
    eval_accuracy = accuracy_score(trues, y_preds)
    eval_recall = recall_score(trues, y_preds, average='macro')
    eval_precision = precision_score(trues, y_preds, average='macro')
    eval_f1 = f1_score(trues, y_preds, average='macro')
    result = {
    ...
    if is_write_preds:
        out_data = {'logits': predicts, 'labels':trues, 'preds': y_preds}
        with open(os.path.join(args.model_path, "test_pred.pkl"), 'wb') as f:
...
if __name__ == '__main__':
    ...
    word2vec = Word2Vec.load(os.path.join(args.data_path, "train/embedding/node_w2v_{}".format(args.encode_dim))).wv
    ...
    if args.do_train:
        ...
        for epoch in range(EPOCHS):
            ...
            for i in tqdm(range(0, len(train_data), BATCH_SIZE),
                          desc="Training: Epoch {} Loss: {}".format(epoch, avg_loss)):
                ...
                model.hidden = model.init_hidden()
                loss, output = model(train_inputs, Variable(train_labels))
    ...
    if args.do_test:
        model.load_state_dict(torch.load(os.path.join(save_path, "{}".format("model.bin"))))
        _, test_result = evaluate(os.path.join(args.data_path, 'test/blocks.pkl'), args.eval_batch_size, is_write_preds=True)
        print(test_result)
