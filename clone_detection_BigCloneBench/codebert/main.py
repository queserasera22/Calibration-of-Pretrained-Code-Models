...
from utils import set_seed, load_and_cache_examples, TextDataset
...
def train(args, model, tokenizer):
    ...
    for idx in range(0, int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(bar):
            ...
            model.train()
            loss, predicts = model(inputs, labels)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                # 一个epoch 记录1次训练集和验证集上的performance，并保存模型， 其中args.save_steps=len(train_dataloader)
...
def evaluate(args, model, tokenizer, eval_data_file, eval_when_training=False, return_preds=False):
    ...
    logits = F.softmax(torch.tensor(logits)).cpu().numpy()
