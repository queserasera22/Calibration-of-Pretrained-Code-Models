...
class TextDataset(Dataset):
    ...
    def __getitem__(self, item):
        return torch.tensor(self.examples[item].input_ids), torch.tensor(self.examples[item].label), \
               torch.tensor(self.examples[item].index)
...
def convert_examples_to_features(code1_tokens, code2_tokens, label, tokenizer, args, index):
    ...
    return InputFeatures(source_tokens, source_ids, label, index)


def get_example(item):
    url1, url2, label, tokenizer, args, url_to_code, index = item
    ...
    return convert_examples_to_features(code1, code2, label, tokenizer, args, index)
