...
def convert_examples_to_features(item):
    ...
    for url in [url1, url2]:
        if url not in cache:
            ...
            # extract data flow
            dfg, index_table, code_tokens = extract_dataflow(func, 'java')
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train'):
        ...
        folder = '/'.join(file_path.split('/')[:-1])  # 寰楀埌鏂囦欢鐩綍
...
def test(args, model, tokenizer, is_wirte_preds=False):
    # build dataloader
...
def main():
    ...
    if args.do_test:
        ...
        model.load_state_dict(torch.load(output_dir))
        model.to(args.device)
        results = test(args, model, tokenizer, is_wirte_preds=True)
